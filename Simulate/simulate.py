...
def main():
    ...
    score = 0
    # when False, the pattern is playing. whwn True, waiting for the player to click a colored button
    ...
    while True: # main game loop
        ...
        if not waitingForInput:
            ...
            if clickedButton and clickedButton == pattern[currentStep]:
                # pushed the correctbutton
                flashButtonAnimation(clickedButton)
                currentStep+=1
                lastClickTime = time.time()

                if currentStep == len(pattern):
                    # pushed the last button int the pattern
...
def checkForQuit():
    ...
    for event in pygame.event.get(KEYUP): # get all keyup events
        if event.key == K_ESCAPE:
            terminate() # terminate of the keyup event was for the esc key
    ...
    for i in range(3): # do the flash 3 times
        for start, end, step in ((0,255,1),(255,0,-1)):
            ...
            for alpha in range(start, end, animationSpeed*step): # animation loop
                # alpha means transperancy. 255 is opaque, 0 in invisible
                checkForQuit()
                flashSurf.fill((r,g,b,alpha))
                DISPLAYSURF.blit(origSurf,(0,0))
